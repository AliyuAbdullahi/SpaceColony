package com.lek.arcade.core.logger
...
object Console {
     ...
     */
    fun warn(message: String, tag: String = "DATA", isAndroidEnvironment: Boolean = false) {
        if (isAndroidEnvironment.not()) {
            Log.w(tag, message)
        } else {
     ...
     * @param isAndroidEnvironment is a Flag to check if in test environment, it should be set to
     * true if in testing environment
     */
    fun error(message: String, isAndroidEnvironment: Boolean = false) {
